...
> 💡 **大一新生实现辅助指引：Radix Tree 的“分裂 (Split)”与“插入”逻辑**
> 
> 在手写 `insert(tokens, value)` 时，你会遇到三种情况：
> 1. **完全不匹配**：直接作为一个新子节点挂在当前节点下。
> 2. **完全匹配**：当前节点已存在完全一致的 key，直接更新其 `value` 并增加 `ref_count`。
> 3. **部分匹配（需要分裂）**：新插入的 `tokens` 与已有的某个子节点的 `key` 有公共前缀，但又不完全相同。
> 
> * **分裂的物理图景**：
>   假设树中已有一个子节点 A，其 `key = [1, 2, 3, 4]`。
>   现在你想插入 `[1, 2, 5, 6]`。
>   1. 找到公共前缀长度 `split_idx = 2` (即 `[1, 2]`)。
>   2. **分裂节点 A**：创建一个新的节点 B，把 A 的后缀 `[3, 4]` 以及 A 原本的 `value` 和 `children` 都转移给 B。
>   3. **更新节点 A**：将 A 的 `key` 缩短为 `[1, 2]`，清空 A 原有的子节点，并把 B 挂为 A 的唯一子节点。此时 A 变成了一个“公共祖先”节点。
>   4. **创建新分支**：创建另一个新节点 C，`key = [5, 6]`，挂在 A 之下。
> 
> * **插入与分裂的实现思路**：
>   ```python
>   def insert(self, tokens, value):
>       # 1. 沿树匹配，找到匹配前缀最多的 TreeNode 和命中的 token 长度 split_idx
>       # 2. 如果匹配长度小于已有子节点的 key 长度，说明在此子节点处发生了“分叉”：
>       #    - 比如子节点 key 是 [1, 2, 3, 4]，但我们只匹配了前 2 个 token [1, 2]
>       #    - 提取公共前缀: common = key[:split_idx]
>       #    - 提取已有的后缀: suffix = key[split_idx:]
>       #    - 创建分裂节点 split_node，其 key = suffix，继承原节点的 value 和 children
>       #    - 原节点 key 更新为 common，children 清空并把 split_node 挂上去
>       # 3. 将新 tokens 的剩余部分（后缀 [5, 6]）创建为另一个新节点，也挂在原节点下
>       # 4. 更新节点的引用计数和最后访问时间
>   ```
