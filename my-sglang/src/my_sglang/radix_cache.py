...
每个节点同时保存等长的 ``key``（token）和 ``value``（KV
...
class TreeNode:
    ...
    key: tuple[int, ...]  # 标准 TreeNode.key 的简化 token tuple。
    value: tuple[int, ...]  # 标准 TreeNode.value 的简化 KV slot tuple。
    parent: TreeNode | None = None  # root 的 parent 为 None。
    children: dict[int, TreeNode] = field(default_factory=dict)  # 首 token -> 子边。
    last_access_time: float = field(default_factory=time.monotonic)  # LRU 时间。
    lock_ref: int = 0  # 与标准 SRT 同名；大于 0 时不能淘汰。
...
class PrefixMatch:
    ...
    last_node: TreeNode  # 命中路径的终点。
    matched_nodes: tuple[TreeNode, ...] = ()  # 依次经过的真实节点。
...
class MiniRadixCache:
    ...
    def __init__(self, max_slots: int | None = None, *, page_size: int = 1):
        ...
        self.root_node = TreeNode(key=(), value=())
    ...
    def match_prefix(
        ...
        *,
        ...
        while remaining:
            ...
            prefix_len = _common_prefix_len(child.key, remaining)
            if prefix_len == 0:
            ...
            if prefix_len < len(child.key):
                ...
                matched_slots.extend(node.value)
    ...
    def pin_nodes(self, nodes: list[TreeNode] | tuple[TreeNode, ...]) -> None:
    ...
    def release_nodes(self, nodes: list[TreeNode] | tuple[TreeNode, ...]) -> None:
    ...
    def _split_node(self, child: TreeNode, split_len: int) -> TreeNode:
        ...
        prefix_key = child.key[:split_len]
        prefix_slots = child.value[:split_len]
        suffix_key = child.key[split_len:]
        suffix_slots = child.value[split_len:]
    ...
    def _find_lru_evictable_leaf(self) -> TreeNode | None:
        """遍历所有未锁定叶子，返回最久未访问的节点。"""
        victim: TreeNode | None = None
        stack = list(self.root_node.children.values())
        while stack:
            node = stack.pop()
            stack.extend(node.children.values())
    ...
    def _remove_leaf(self, node: TreeNode) -> tuple[int, ...]:
