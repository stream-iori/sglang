...
from my_sglang.overlap_scheduler import MiniFutureMap, MiniOverlapScheduler
...
def _forward(mode: ForwardMode, rows: tuple[int, ...]):
    from my_sglang.models import BatchForward
    reqs = tuple(make_req(f"r{i}") for i in range(len(rows)))
    return BatchForward(mode, reqs, tuple((1,) for _ in rows), rows, tuple((0,) for _ in rows), tuple(1 for _ in rows))
