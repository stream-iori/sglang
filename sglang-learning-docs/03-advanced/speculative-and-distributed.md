...
> ⚠️ **大一新生进阶思考：既然 TP（张量并行）能横切模型，为什么不直接拆到 100 张卡上算（TP=100）？**
> 
> * **卡间通信的瓶颈**：
>   TP 是把**每一层**的矩阵运算切开给不同 GPU。每个 GPU 算完自己的局部结果后，必须通过网络进行一次 **All-Reduce**（卡间数据同步合并），把各自的局部结果加起来，然后才能进入下一层的计算。大模型有上百层，前向传播一次要进行**数百次卡间数据同步**！
> * **“抄作业”的比喻**：
>   这就像 4 个人合作写一份作业。每个人负责写每行字的其中一个偏旁部首。**每写完一行字，这 4 个人就必须停下来，把各自写的拼在一起，互相抄写对齐一份，然后再去写下一行。**
>   * 如果只有 2 个人，转个头对一下就能抄完，效率很高。
>   * 如果有 100 个人，每写一行都要等 100 个人全部连线、对齐、抄完。结果绝大部分时间都花在“对齐抄写（网络通信）”上，真正的“写字（GPU 计算）”时间微乎其微，效率反而暴跌。
> * **物理互联限制**：
>   只有在同一台服务器内、通过极高速的 **NVLink 互联**（如 NVIDIA H100 卡间 900 GB/s 的专用通道）的 GPU 之间才适合做 TP。如果是跨机器（走普通的万兆网卡或 PCIe），TP 的数据交换延迟会彻底卡死 GPU。因此，单机 8 卡环境下我们最多做 TP=8；如果有多台机器，我们通常会组合使用 **DP（数据并行）** 或 **PP（流水线并行）**。
